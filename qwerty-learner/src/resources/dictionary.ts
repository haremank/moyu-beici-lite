...
// moyu-lite 壳注册的自定义词书：运行时同步读取 /dicts/custom/index.json 合并进词书列表。
// index.json 由 moyu-lite 主进程在导入词书时生成。
let customDicts: DictionaryResource[] = []
try {
  const xhr = new XMLHttpRequest()
  xhr.open('GET', './dicts/custom/index.json', false)
  xhr.send(null)
  if (xhr.status === 200 && xhr.responseText) {
    const parsed = JSON.parse(xhr.responseText)
    if (Array.isArray(parsed)) customDicts = parsed
  }
} catch {
  // 无自定义词书时静默跳过
}
